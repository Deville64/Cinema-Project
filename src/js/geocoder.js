...
(function (global, factory) {
  ...
  var Geosearch = leaflet.Control.extend({
      ...
      for (var i = 0; i < suggestions.length; i++) {
        ...
        if (suggestionTextArray.indexOf(suggestion.text) === -1) {
          ...
          suggestionItem.addEventListener('click', function (event) {
            var getInput = document.getElementsByClassName("geocoder-control-input leaflet-bar geocoder-control-expanded");
            var getSuggestions = document.getElementsByClassName("geocoder-control-list");
            console.log(event.target.innerHTML);
            console.log(getInput[0].value);
            getInput[0].value = event.target.innerHTML;
            getSuggestions[0].style.display = 'none'
          });
        } else {
    ...
    onAdd: function (map) {
      ...
      this._map = map;
      this._wrapper = leaflet.DomUtil.create('div', 'geocoder-control geocoder-control-expanded');
      this._input = leaflet.DomUtil.create('input', 'geocoder-control-input leaflet-bar geocoder-control-expanded', this._wrapper,);
      this._input.autocomplete = "off";
      this._input.name = 'address';
